...
module.exports = async (req,res)=>{
  ...
  try{
    const ur=await fetch(`${process.env.SUPABASE_URL}/rest/v1/users?select=phone,role&auth_user_id=eq.${authUser.id}&limit=1`,{headers:common});
    const users=ur.ok?await ur.json():[]; if(!users.length)return res.status(403).json({ok:false,message:'Профиль табылмады'});
    const table=kind==='product'?'products':'listings',isAdmin=users[0].role==='admin'||isAdminEmail(authUser.email);
    ...
    const r=await fetch(`${process.env.SUPABASE_URL}/rest/v1/${table}?${filter}`,{method:'DELETE',headers:{...common,Prefer:'return=representation'}});
