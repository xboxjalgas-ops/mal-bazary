...
module.exports=async(req,res)=>{
 ...
 try{
  const ur=await fetch(`${process.env.SUPABASE_URL}/rest/v1/users?select=phone&auth_user_id=eq.${authUser.id}&limit=1`,{headers:headers()});
  ...
  const r=await fetch(`${process.env.SUPABASE_URL}/rest/v1/listings?id=eq.${encodeURIComponent(id)}&seller_phone=eq.${encodeURIComponent(users[0].phone)}`,{method:'PATCH',headers:headers({'Content-Type':'application/json',Prefer:'return=representation'}),body:JSON.stringify(patch)});
