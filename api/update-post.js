...
module.exports=async(req,res)=>{
 ...
 try{
  ...
  if(b.title!==undefined){
   ...
   const isAdmin=users[0].role==='admin'||isAdminEmail(authUser.email);const moderation=isAdmin?{status:'approved',risk:'low',reason:'Админ жаңартты'}:await moderateContent(b);if(moderation.status==='rejected')return res.status(400).json({ok:false,message:`Модерация өткізбеді: ${moderation.reason}`});
   const region=String(b.region||'').trim().slice(0,80),district=String(b.district||'').trim().slice(0,100),village=String(b.village||'').trim().slice(0,100),d=b.animal_details||{};const animal_details={breed:String(d.breed||'').slice(0,80),age:String(d.age||'').slice(0,50),weight:String(d.weight||'').slice(0,50),health:String(d.health||'').slice(0,120),documents:String(d.documents||'').slice(0,300)};Object.assign(patch,{title,description,location,type,price,region,district,village,images:b.images||[],animal_details,moderation_status:isAdmin?'approved':moderation.status});
  }
  if(Object.keys(patch).length===1)return res.status(400).json({ok:false,message:'Өзгеріс жоқ'});
  const isAdmin=users[0].role==='admin'||isAdminEmail(authUser.email);const filter=isAdmin?`id=eq.${encodeURIComponent(id)}`:`id=eq.${encodeURIComponent(id)}&seller_phone=eq.${encodeURIComponent(users[0].phone)}`;
